...
import * as BlocklyActions from './../authoring/edit/blockly/store/blockly.actions';
...
export class AppEffects {
    ...
    .pipe(
      ...
      mergeMap(() => {
        return [
          ...
          { type: BlocklyActions.INITAIL_BLOCKLY_STATE },
        ];
      })
    );
  @Effect()
  tryInitailProjectEffects = this.action$
    .pipe(
      ofType(AppActions.TRY_INITAIL_PROJECT_STATE),
      mergeMap(() => {
        return [
          { type: AppActions.INITAIL_APP_STATE },
          { type: AuthoringStageActions.INITAIL_AUTHORING_STAGE_STATE },
  ...
  @Effect()
  tryInitailStageEffects = this.action$
    .pipe(
      ofType(AppActions.TRY_INITAIL_STAGE_STATE),
      mergeMap(() => {
        return [
          { type: AuthoringStageActions.INITAIL_AUTHORING_STAGE_STATE },
          { type: ManagementActions.INITAIL_MANAGEMENT_STATE },
